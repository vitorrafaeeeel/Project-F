import { X, Receipt, Wand2, Sparkles, Plus, Trash2, ListPlus } from 'lucide-react';
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.01" required min="0.01" placeholder="0.00" readOnly={hasItems} value={hasItems ? itemsTotal.toFixed(2) : newExpense.amount} onChange={(e) => setNewExpense({...newExpense, amount: e.target.value})} className={`w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-red-500 focus:ring-red-500 dark:bg-gray-700 dark:text-white p-2.5 border bg-white dark:bg-gray-700 ${hasItems ? 'opacity-70 cursor-not-allowed' : ''}`}/>
            </div>
          ...
          <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={addItem} className="text-xs font-semibold text-red-600 hover:text-red-700 flex items-center gap-1"><ListPlus size={14} /> Adicionar item</button>
            </div>
            {hasItems && (
              <div className="space-y-2">
                {items.map((it, idx) => (
                  <div key={idx} className="flex gap-2 items-start bg-gray-50 dark:bg-gray-900/40 p-2 rounded-lg border border-gray-200 dark:border-gray-700">
                    <input type="text" required placeholder="Descrição" value={it.desc} onChange={(e) => updateItem(idx, 'desc', e.target.value)} className="flex-1 min-w-0 rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-red-500 focus:ring-red-500 dark:bg-gray-700 dark:text-white p-2 border bg-white dark:bg-gray-700 text-sm"/>
                    <input type="number" step="0.01" min="0.01" required placeholder="0.00" value={it.amount} onChange={(e) => updateItem(idx, 'amount', e.target.value)} className="w-24 rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-red-500 focus:ring-red-500 dark:bg-gray-700 dark:text-white p-2 border bg-white dark:bg-gray-700 text-sm"/>
                    <select value={it.category} onChange={(e) => updateItem(idx, 'category', e.target.value)} className="w-32 rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-red-500 focus:ring-red-500 dark:bg-gray-700 dark:text-white p-2 border bg-white dark:bg-gray-700 text-sm">
                      {categoryOptions}
                    </select>
