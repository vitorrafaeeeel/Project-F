      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6 relative border border-gray-200 dark:border-gray-700">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nova Meta (R$/mês)</label><input type="number" inputMode="decimal" step="0.01" required min="0" value={editInvModal.monthlyAmount} onChange={(e) => setEditInvModal({...editInvModal, monthlyAmount: e.target.value})} className="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white p-3 border bg-white dark:bg-gray-700"/></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Rendimento (%/mês)</label><input type="number" inputMode="decimal" step="0.01" value={editInvModal.interestRate} onChange={(e) => setEditInvModal({...editInvModal, interestRate: e.target.value})} className="w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white p-3 border bg-white dark:bg-gray-700"/></div>
          </div>
