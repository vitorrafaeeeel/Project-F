...
import {
  DollarSign, ArrowDownCircle, Wallet, BarChart3, TrendingUp, ArrowDownRight, ArrowUpRight
} from 'lucide-react';
import { formatCurrency } from '../../../shared/lib/currency.js';

export const DashboardPage = memo(({ projections, data }) => {
  const maxChartValue = useMemo(() => {
    if (!projections?.timeline || projections.timeline.length === 0) return 1000;
    const values = projections.timeline.flatMap(t => [
      Math.max(0, Number(t.netBalance) || 0),
      Math.max(0, Number(t.totalInvestments) || 0)
    ]);
    const maxVal = Math.max(...values, 0);
    if (maxVal <= 0) return 1000;
    // Margem suave de 15% acima do maior valor para respiro no topo
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-colors duration-300">
        <div className="p-5 sm:p-6 border-b border-gray-100 dark:border-gray-700">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <TrendingUp size={20} className="text-blue-500 dark:text-blue-400" /> Evolução Projetada (12 Meses)
          </h3>
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300 mt-1 leading-relaxed">
            A projeção assume que nos meses futuros você atingirá o seu Planejamento de Gastos.
          </p>
        </div>
